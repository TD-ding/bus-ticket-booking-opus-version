...
- 班次查询：按出发城市 / 到达城市 / 日期筛选
- 在线购票：**防超卖**，并发下单不会卖超座位数
- 我的订单：查看、取消（取消后自动恢复余票）

### 管理员后台
- 班次管理：增删改查，含余票 / 座位数 / 上下架管理
- 订单管理：查看全部订单、代取消（恢复余票）
- 用户管理：查看用户列表、删除用户

## 防超卖实现

下单在一个 SQLite **事务**内完成，余票扣减使用带条件的原子更新：
...
只有当 `changes === 1`（即余票确实足够）时才会写入订单，否则回滚并返回「余票不足」。
配合 better-sqlite3 的串行化事务，10 个并发请求抢 3 个座位时，**精确成功 3 单、失败 7 单，余票归零且绝不为负**（见单元测试）。
...
覆盖：注册/登录、重复用户名、班次查询过滤、未登录拦截、**并发下单防超卖**、取消恢复余票、管理员权限校验。测试使用独立的 `test/test.db`，不影响开发数据。
...
│       ├── auth.js        # 注册 / 登录 / 当前用户
│       ├── trips.js       # 班次查询（公开）
│       ├── orders.js      # 下单 / 我的订单 / 取消（防超卖）
│       └── admin.js       # 班次/订单/用户管理（管理员）
├── public/
